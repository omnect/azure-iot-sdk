...
impl<'c> IotHubModuleClient<'c> {
    ...
    fn twin_callback(&mut self, data: &[u8], state: DEVICE_TWIN_UPDATE_STATE) {
        ...
        (self.callback)(
            IotHubModuleEvent::Twin(settings),
            IotHubModuleState::TwinState(state),
        );
    }
    ...
    pub fn new(
        callback: impl FnMut(IotHubModuleEvent, IotHubModuleState) + 'c,
        ...
        let client = Box::new(IotHubModuleClient {
            handle,
